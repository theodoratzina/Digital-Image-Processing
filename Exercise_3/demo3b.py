import matplotlib.pyplot as plt
from scipy.io import loadmat
from image_to_graph import image_to_graph
from n_cuts import n_cuts_recursive
from n_cuts import calculate_n_cut_value


# Load the images data
data = loadmat("dip_hw_3.mat")
img_array_1 = data["d2a"]  
img_array_2 = data["d2b"]

# Create new figures for the original images
fig1 = plt.figure(1, figsize=(8, 8))  
plt.imshow(img_array_1, cmap='gray')
plt.title('Original Image d2a') 
plt.axis('off')

fig2 = plt.figure(2, figsize=(8, 8))
plt.imshow(img_array_2, cmap='gray')
plt.title('Original Image d2b')
plt.axis('off')

# Convert the images to graphs (affinity matrix)
affinity_mat_1 = image_to_graph(img_array_1)
affinity_mat_2 = image_to_graph(img_array_2)

# Perform the recursive n-cuts for one step
print(f"\n🔹 Recursive Normalized-cuts for one step")
T1 = 2
T2 = 0.1
labels_1 = n_cuts_recursive(affinity_mat_1, T1, T2)
labels_2 = n_cuts_recursive(affinity_mat_2, T1, T2)

# Plotting the results
fig1 = plt.figure(3)
plt.title(f"Recursive Normalized-cuts on Image d2a for one step \n"
          f"T1: {T1}, T2: {T2} \n"
          f"N-cut value: {calculate_n_cut_value(affinity_mat_1, labels_1)}")
plt.imshow(labels_1.reshape(img_array_1.shape[:2]), cmap="nipy_spectral")
plt.axis("off")

fig2 = plt.figure(4)
plt.title(f"Recursive Normalized-cuts on Image d2b for one step \n"
          f"T1: {T1}, T2: {T2} \n"
          f"N-cut value: {calculate_n_cut_value(affinity_mat_2, labels_2)}")
plt.imshow(labels_2.reshape(img_array_2.shape[:2]), cmap="nipy_spectral")
plt.axis("off")

plt.show()