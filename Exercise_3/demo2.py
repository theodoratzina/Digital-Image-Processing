...
# Perform the spectral clustering for various values of k
for k in [2, 3, 4]:
    labels_1 = spectral_clustering(affinity_mat_1, k)
    labels_2 = spectral_clustering(affinity_mat_2, k)
    
    print(f"\n🔹 Spectral Clustering of d2a for k = {k}")
    print(labels_1)
    print("-" * 40)
    print(f"\n🔹 Spectral Clustering of d2b for k = {k}")
    print(labels_2)
    print("-" * 40)

    # Plotting the results
    fig3 = plt.figure(3)
    plt.title(f"Spectral Clustering on Image d2a (k={k})")
    plt.imshow(labels_1.reshape(img_array_1.shape[:2]), cmap="nipy_spectral")
    plt.axis("off")

    fig4 = plt.figure(4)
    plt.title(f"Spectral Clustering on Image d2b (k={k})")
